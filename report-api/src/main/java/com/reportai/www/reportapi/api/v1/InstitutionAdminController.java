...
public class InstitutionAdminController {
    ...
    public ResponseEntity<InstitutionResponseDto> getInstitutionById(@PathVariable UUID id) {
        Optional<Institution> institution = adminService.getInstitutionById(id);
        if (institution.isEmpty()) {
            throw new HttpInstitutionNotFoundException();
        }
        InstitutionResponseDto response = InstitutionMappers.convert(institution.get());
